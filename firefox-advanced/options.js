(function(w, d, b, f) {
	...
	d.addEventListener("click", (e) => {
		...
		else if (e.target.className == "source-icon-button") {
			try {
				const domain = new URL(targetInput.parentElement.parentElement.querySelector("[name='source[]']").value || '');
				const url = 'https://www.google.com/s2/favicons?sz=32&domain=' + domain.hostname;
				targetImage.src = url;
				targetInput.value = url;
				d.querySelector("[name='custom']").value = url;
			}
			catch (e) {
				// console.log('error', e);
				d.querySelector("[name='custom']").value = '';
		...
		else if (e.target.className == "destination-icon-button") {
			try {
				const domain = new URL(targetInput.parentElement.parentElement.querySelector("[name='target[]']").value || '');
				const url = 'https://www.google.com/s2/favicons?sz=32&domain=' + domain.hostname;
				targetImage.src = url;
				targetInput.value = url;
				d.querySelector("[name='custom']").value = url;
	...
	d.getElementById("export-button").addEventListener("click", () => {
		b.storage.sync.get(null).then((res) => {
			...
			const filename = 'location-switcher-advanced-addon.json';
